...
contact_scheduler.add_phase([], 0.3)
contact_scheduler.add_phase(["l_foot", "r_foot"], 1.0)
...
for contact_phase_fnames in frame_contact_seq:
    ...
    # stage_node.costs_list.extend([ConfigurationCost(q.copy()[7:], np.eye(robot.model.nv - 6) * 1e-3)])
...
opti.save_solution("jump")

K = len(result["nodes"])
dts = [result["nodes"][k]["dt"] for k in range(K)]
qs = [result["nodes"][k]["q"] for k in range(K)]
vqs = [result["nodes"][k]["v"] for k in range(K)]
aqs = [result["nodes"][k]["a"] for k in range(K)]
